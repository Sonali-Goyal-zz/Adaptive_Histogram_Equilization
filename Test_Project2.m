...
chang = imread('chang.jpg');
crowd = imread('crowd.jpg');
portal = imread('portal.jpg');
...
toc
tic;
he_chang = histoeq( chang, 256, 0, 256 );
toc
tic;
he_crowd = histoeq( crowd, 256, 0, 256 );
toc
tic;
he_portal = histoeq( portal, 256, 0, 256 );
toc
tic;
...
bar(histogram(portal, 256, 0, 256));
set(gca, 'xlim',[0 256]);
waitforbuttonpress;
...
univ = rgb2gray(univ);
size(chang, 3)
chang = rgb2gray(chang);
size(crowd, 3)
crowd = rgb2gray(crowd);
size(portal, 3)
portal = rgb2gray(portal);
size(walle, 3)
...
for ii=i
    ...
    toc;
    tic;
    ahe_chang = [ahe_chang; AHE( chang, 256, ii )];
    toc;
    tic;
    ahe_crowd = [ahe_crowd; AHE( crowd, 256, ii )];
    toc;
    tic; 
    ahe_portal = [ahe_portal; AHE( portal, 256, ii )];
    toc;
    tic;
...
for ii=i
    for k=0.4:0.2:0.8
        ...
        toc
        tic;
        clahe_chang = [clahe_chang; CLAHE( chang, 256, k, ii )];
        toc
        tic;
        clahe_crowd = [clahe_crowd; CLAHE( crowd, 256, k, ii )];
        toc
        tic;
        clahe_portal = [clahe_portal; CLAHE( portal, 256, k, ii )];
        toc
        tic;
